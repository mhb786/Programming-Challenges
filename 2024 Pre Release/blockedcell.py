...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            self.RemoveBlocks(Patterns)
            if self.__SymbolsLeft == 0:
                Finished = True
        print()
        self.DisplayPuzzle()
        print()
        print(self.FindPatterns())
        return self.__Score
    ...
    def FindPatterns(self):
        ...
        for StartRow in range(5, 2, -1):
            for StartColumn in range(1, 4):
                ...
                for P in self.__AllowedPatterns:
                    if P.MatchesPattern(PatternString, CurrentSymbol):
                        arr.append([CurrentSymbol, StartRow, StartColumn])
        return arr

    def RemoveBlocks(self, Patterns):
        CurrentPatterns = self.FindPatterns()
        for count, Pattern in enumerate(Patterns):
            try:
                if Pattern != CurrentPatterns[count]:
                    CurrentSymbol, StartRow, StartColumn = Pattern[0], Pattern[1], Pattern[2]
                    self.__GetCell(StartRow, StartColumn).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow, StartColumn + 1).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow, StartColumn + 2).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 1, StartColumn + 2).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 2, StartColumn + 2).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 2, StartColumn + 1).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 2, StartColumn).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 1, StartColumn).RemoveFromNotAllowedSymbols(CurrentSymbol)
                    self.__GetCell(StartRow - 1, StartColumn + 1).RemoveFromNotAllowedSymbols(CurrentSymbol)
            except:
                pass
