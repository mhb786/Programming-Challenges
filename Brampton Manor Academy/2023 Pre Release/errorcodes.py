...
def UpdateSymbolTable(SymbolTable, ThisLabel, LineNumber):
    if ThisLabel in SymbolTable:
        print("Error Code 3: Opcode not in SymbolTable")
    else:
    ...
    if len(Instruction) > 9:
        ...
        if Operation in OpCodeValues:
            ...
            if Operation != EMPTY_STRING:
                print("Error Code 5: the operand is EmptyString")
                Memory[0].OpCode = "ERR"
    return Memory
    ...
    while not Finished:
        ...
        if MenuOption == 'L':
            ...
            if SourceCode[0] == EMPTY_STRING:
                print("Error Code 7: number of lines of code is empty")
            else:
                DisplaySourceCode(SourceCode)
        elif MenuOption == 'E':
            if SourceCode[0] == EMPTY_STRING:
                print("Error Code 8: number of lines of code is empty")
            else:
                SourceCode = EditSourceCode(SourceCode)
                Memory = ResetMemory(Memory)
        elif MenuOption == 'A':
            if SourceCode[0] == EMPTY_STRING:
                print("Error Code 9: number of lines of code is empty")
            else:
                Memory = Assemble(SourceCode, Memory)
        elif MenuOption == 'R':
            if Memory[0].OperandValue == 0:
                print("Error Code 10: operand value is 0")
            elif Memory[0].OpCode == "ERR":
                print("Error Code 11: opcode is ERR")
            else:
