...
def ResetDataStructures():
    """
    parameters:
    description: creates 4 empty arrays
...
def LoadPuzzleFile(PuzzleName, Puzzle):
    ...
    description: read through file and fill puzzle file
...
def LoadSolution(PuzzleName, Solution):
    """
    parameters: string, array
    description: fills in solution array with data from solution file
    return type: array, boolean
    """
    OK = True
    try:
...
def ResetAnswer(PuzzleName, Answer):
    """
    parameters: string, array
    description: resets answer to empty string
    return type: array
    """
    Answer[0] = PuzzleName
    Answer[1] = "0"
...
def TransferPuzzleIntoGrid(PuzzleName, PuzzleGrid, Puzzle, Answer):
    ...
    description: transfers data from puzzle into puzzle grid array
    return type: array, array, boolean
    """
    OK = True
    try:
        Line = 0
...
def LoadPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
    ...
    description: enter the puzzle name and load the puzzle file and solution
    return type:
    """
...
def TransferAnswerIntoGrid(PuzzleGrid, Answer):
    ...
    description: transfers data from answer to puzzle grid
    return type:
    """
...
def LoadPartSolvedPuzzle(PuzzleGrid, Puzzle, Answer, Solution):
    """
    parameters: array, array, array, array
    description: loads puzzle then fills in missing numbers using data from the array
    return type: array, array, array, array
    """
...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer):
    ...
    description: recieves input from user and fills in puzzle grid
...
def GetMenuOption():
    """
    description: recieves the users choice
    return type: string
    """
    Choice = EMPTY_STRING
    while len(Choice) != 1:
...
def KeepPuzzle(PuzzleGrid, Answer):
    """
    parameters: array, array
    description: creates a new file with the puzzle that is being worked on
    """
    if PuzzleGrid[0][0] != 'X':
        print("No puzzle loaded")
    else:
...
def CheckSolution(PuzzleGrid, Answer, Solution):
    """
    parameters: array, array, array
    description: checks if every entry in the puzzle grid matches the solution and tells you where you made any errors
...
def CalculateScore(Answer, ErrorCount):
    ...
    description: calculates the score
    return type: array
    """
...
def DisplayResults(Answer):
    """
    parameters: array
    description: displays your score and the solution
    """
    if int(Answer[2]) > 0:
