...
def CheckIfEntered(PuzzleGrid, Row, Column, Digit):
    ...
    for x in range(GRID_SIZE + 1):
        if PuzzleGrid[Row][x] == Digit or PuzzleGrid[x][Column] == Digit:
            print('This is a duplicate number in the row/column\n')
            check = False

    location = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    BoxRow = [location.index(x) for x in location if Row in x][0]
    BoxColumn = [location.index(x) for x in location if Column in x][0]

    for R in range(location[BoxRow][0] - 1, location[BoxRow][2]+1):
        for C in range(location[BoxColumn][0] - 1, location[BoxColumn][2]+1):
            if PuzzleGrid[R][C] == Digit:
                print('There is a duplicate number in the same 3x3 grid\n')
                check = False
    ...
    if PuzzleGrid[0][0] != 'X':
        ...
        while CellInfo != EMPTY_STRING:
            ...
            if InputError:
                ...
                for x in Puzzle:
                    if CellInfo[:-1] == x[:-1]:
                        print('This space already been prefilled - cannot change\n')
                        br = False
                        break

                if br and CheckIfEntered(PuzzleGrid, Row, Column, Digit):
                    PuzzleGrid[Row][Column] = Digit
